/**
 * A throwaway local Postgres for the storage integration test — the same
 ...
 * that a package's tests stay self-contained. Admin CREATE/DROP DATABASE go
 * through Bun's SQL (the store's own driver) rather than adding a `pg` dep.
 */
...
export async function createTestDatabase(baseUrl: string): Promise<TestDatabase> {
  const name = `storage_test_${randomUUID().replace(/-/g, '')}`;
...
export const startTestPostgres = (): TestPostgres | undefined => {
  ...
  if (initdb === undefined || pgCtl === undefined) {
    if (process.env['CI'] !== undefined) {
      throw new Error(
        'CI is set but no Postgres is available for the storage integration test: neither ' +
  ...
  const dataDir = fs.mkdtempSync(path.join(baseDir, 'prisma-composer-storage-pg-'));
