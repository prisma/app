...
/**
 * Stops and removes every `pcdev-<app>-*` local Postgres instance. A missing
 * `prisma` bin means the app never used postgres — nothing to remove.
 * `startDir` is the caller's `process.cwd()` (kept out of this package's own
 * env/cwd reads elsewhere — only bin resolution is legitimately cwd-relative).
 */
