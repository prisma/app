/**
 * Discovery (walk-up) + real c12 evaluation of a `prisma-app.config.ts` in a
...
function makeTree(): string {
  const dir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'prisma-app-cli-config-')));
  tmpDirs.push(dir);
  return dir;
}
...
describe('validateConfigShape() — field-by-field CliErrors', () => {
  const configPath = '/repo/app/prisma-app.config.ts';
