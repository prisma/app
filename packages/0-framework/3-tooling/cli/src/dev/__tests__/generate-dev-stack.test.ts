...
describe('renderDevStackFile()', () => {
  test('renders the config + app imports (relative), resolved dev providers, localState(), no report', () => {
    ...
    // The one orchestration point (spec § 3 REVISED): containers deserialized,
    // dev thunks resolved by top-level await, providers + state passed
    // explicitly — and no `dev` flag, which no longer exists on LowerOptions.
    expect(content).toContain(
      'const containers = deserializeContainers(config.extensions, process.env);',
    );
    expect(content).toContain('const resolved = await resolveDevDescriptors(config);');
    ...
    expect(content).toContain('providers: devProviders(resolved, containers, devDir)');
