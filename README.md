...
applications composed of components. A component — a **System** — owns its Services
and Resources and exposes typed **Inputs** and **Outputs**; you build your app by
connecting one System's Outputs to another's Inputs, and at runtime the framework
injects the dependencies that satisfy them. The whole app is itself a System — the
...
Each box above is a **System**: a bounded context that owns some code and data and
is reachable only through a typed boundary. Because nothing reaches inside except
through that boundary, every dependency between Systems is an explicit edge in the
graph. A System is scale-invariant — it composes recursively, and the whole
application is just the outermost System — so the root needs no special construct.

A System wraps two kinds of thing. **Services** are the units that run code — an
...
You can see this in the diagram: every System has a Compute Service, Sales also
...
Systems — only the shared Postgres, used by several Systems at once, does.

> A System is the building block of hexagonal (ports-and-adapters) architecture — a
> bounded context that reaches the world only through typed ports, which is exactly
> what its Inputs and Outputs are. ("Hex," from that hexagon, was this unit's earlier
> working name.)

## Connections

A System declares **Inputs** for what it needs and **Outputs** for what it offers;
a connection joins one System's Output to another's Input. There are two kinds.

**Communication** is one System calling another — either **request/response** (the
storefront calls the sales API) or a **stream** (sales emits *order-placed*, and
delivery consumes it).

**Data** is a System reading and writing a Postgres Resource. The data Input carries
a **contract** — a Prisma Next description of exactly the tables and columns the
System may touch — and a Postgres satisfies the connection only if it meets that
...
reach into another System's tables.

## Topology

The wired-up graph of Systems, Resources, and connections is the **topology**.
...
- **Authoring** — what you write: Systems, Services, Resources, connections.
...
same System runs against managed Postgres in the cloud and a local one in
development, with no change to your code.

## Decisions

- **Prisma App owns composition and borrows everything beneath it.** The System
...
- **Thin core, fat targets.** The core is the System / Input / Output model and the
...
- **No globals.** Application code never reads `process.env` or looks a service up
  by name. Every resource a System uses arrives as an injected, typed dependency.
