...
This package carries its own `prisma-composer.config.ts` (ADR-0017): `prisma-composer
deploy` discovers it by walking up from the fixture entry and evaluates it
with c12, so its static imports of `@prisma/composer-prisma-cloud/control` and
