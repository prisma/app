...
- **The control plane loads through `prisma-composer.config.ts`** — the config
...
- **A reusable System, proven, plus the testing seam** — auth becomes a System
  ...
  (`@prisma/composer/testing`, unit) and `bootstrapService` (`@prisma/composer-cloud/testing`,
  ...
  `docs/design/10-domains/testing.md`; `skills/testing-prisma-composers`; PR #39.

Built on the merged model: unified dependency slots (#21), always-system root
(#22), the `@prisma/composer*` / System naming (#24 = ADR-0014), and
...
- **Fold `@prisma/composer-alchemy` into `@prisma/composer-cloud`.** One consumer, a
