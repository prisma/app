# Hex Composition — Design Notes

The design lives in ADR-0014 and `docs/design/10-domains/hex-composition.md`.
...
1. **Breaking `hex()` reshape, no legacy form.** `hex(name, body)` is replaced
   by `hex(name, { deps?, expose? }, body)` with the new body shape
   (`{ inputs, provision }` in, outputs returned). The two existing call
   sites + fixtures migrate; no 2-arg overload survives. Confirm.
2. **Load validation rule set** (hex-composition.md § Load): dangling
   declared input; missing/unsatisfied expose return; root-deployed hex with
   ...
   its consumers).
3. **Auth-hex v1 `db` posture.** Boundary input (needs the resource-slot work
...
4. **Resource-slot seam.** The parallel resource-decoupling session defines
   the slot type services (and therefore hex boundaries) use for resource
...
- **`InputRef`** — the body's `ctx.inputs` entries are branded wiring values
  carrying (hex identity, input key). During Load, when the enclosing scope
  wires the hex's input, the ref resolves to the actual producer; forwarding
...
- **Body executes during Load only**, exactly once per provision, pure by
  convention (same expectation as today's hex body); nothing composition adds
  runs at runtime.
...
- **`HexNode<D, E>`** carries the boundary types; stays a plain frozen node
...
- **Workspace-package hex** (H3): built via its own `build` script at repo
...
`satisfies()` check, and resource-slot forwarding across a hex boundary now
works (no `never` soft-block). The hex-boundary ADR renumbered to **0014** (#21
took 0009–0013). The pre-unification detail below is retained for history.

Hex-composition rebases onto BOTH, in this order:
...
intersection adding required `ResourceRef<NoInfer<T>>` entries; `loadHex`
...
**PR #22 — always-hex root** (`claude/delete-example-apps-f54df6`). The deploy
root MUST be a hex; a bare `compute()` service is no longer independently
deployable — Load rejects a non-hex root with a "wrap it in a hex" error. This
...
`bundle` shape, `isHexRoot` branching, `resolveBundle` removed — baseline is
bundles-keyed-by-address only); ADR-0007, deploy-cli.md, core-model.md amended;
`examples/makerkit-hello` and the smoke example DELETED, and the e2e workflow's
...
standalone application" — now false under always-hex-root. The operator amends
it on the #22 line (reframing the operational root as always a hex, keeping the
...
- `makerkit-hello` no longer exists — repoint any fixture/smoke reference to
  `examples/storefront-auth` (the surviving hex example) or stand up a minimal
  single-service hex where a smaller fixture is wanted.
...
- Baseline to code against: always-hex root, bundles-keyed-by-address only.
...
- **Type-level cost** of `HexContext`/`Wiring` inference at depth — R6's
