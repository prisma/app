...
New package `@prisma/composer-cron` ships the reusable pieces; an in-repo example
...
1. **`@prisma/composer-cron` package** with the reusable scheduler, the two authoring
...
The router's entry. Mirrors `@prisma/composer-rpc`'s `serve()` exactly — a fetch
...
Dependencies: `@prisma/composer`, `@prisma/composer-cloud` (`compute`), `@prisma/composer-node`
(`node` build adapter), `@prisma/composer-rpc` (`contract`, `rpc`, `serve`), `arktype`.
...
- `defineSchedule({ tick: '2s', mrr: '5s' })` (short intervals so the test is
  ...
  `prisma-composer.config.ts` if it is to be deployable.
...
1. Boots the router's real entry with `bootstrapService` (from
   `@prisma/composer-cloud/testing`) against a fake target, so `POST /rpc/trigger`
