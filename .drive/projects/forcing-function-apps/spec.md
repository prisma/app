...
Ground Prisma Composer's next capabilities in **real applications we
...
- **Depends on the publishing setup** (PR #29): the ports live in their own
  repos and consume published `@prisma/composer*` packages, which makes this
...
- Tracker: [Prisma App: Forcing-Function Apps](https://linear.app/prisma-company/project/prisma-composer-forcing-function-apps-495e5a5c6a0d)
  (Terminal).
...
- Ports may begin against `workspace:`/preview versions of `@prisma/composer*`, but
  the datahub port must end on published versions (that's part of its DoD).

# Project-DoD

- [ ] **datahub deploys via `prisma-composer deploy`** from its own repo, consuming
      published `@prisma/composer*` packages: ingest + web as services, postgres as
