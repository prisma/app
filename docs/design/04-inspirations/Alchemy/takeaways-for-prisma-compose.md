# Alchemy → Prisma Composer takeaways (evolving)

This doc is explicitly **not** "research." It records what we currently believe
Prisma Composer should emulate/adapt from **Alchemy v2**, and it is
...
The goal we're chasing (see `../Convex/takeaways-for-prisma-composer.md`) is the
...
A Component in Prisma Composer wants to be both at once: a deployable
...
- **Unify the two "connect" mechanisms.** Alchemy splits **Layer** (local DI) from
  **Reference** (concrete, address-based, cross-deployment pull from state). A
  Prisma Composer **port** should be one typed interface satisfiable
...
- Do we adopt **Effect** as the framework's substrate? (The thing that makes
  Alchemy "feel like Prisma Composer" is Effect, not the apply
  engine.) → warrants an ADR.
- What is the precise Prisma Composer **Component** primitive that fuses
