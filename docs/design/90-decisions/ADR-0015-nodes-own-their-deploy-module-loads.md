# ADR-0015: Nodes own their deploy-module loads
...
depends on no pack — that is deliberate, so `makerkit` ships knowing nothing
...
needed *only* transitively by an installed hex, under a package manager
configured to disable hoisting, is not visible from core's anchor. The
resolution-failure error still names the fix ("the app, or the hex package that
...
would require resolving from the hex's own location rather than core's — a
future refinement, not part of this decision.

**Identity is unaffected.** Nodes became classes to carry these methods, but
identity still rides the `Symbol.for("makerkit:node")` brand that `isNode()`
checks — never `instanceof`. A node built by a different installed copy of core
(which an installed hex can bring) still validates. The classes are a carrier
...
- The generated stack file (`.makerkit/alchemy.run.ts`) may `import` the target
  ...
  firewall.
- A published hex resolves its own adapters and target the same way, provided
...
- **The CLI constructs `${pack}/target` and resolves it to a path** via
  ...
  reach a build adapter that an installed hex kept internal. Replaced by letting
