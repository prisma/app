...
- [ADR-0022](ADR-0022-a-prisma-app-is-one-project-a-stage-is-a-branch.md) — A Prisma App is one Prisma Cloud Project; Systems are Apps/Databases inside it; a Stage is a Branch, and deploy state is per `(Project, Branch)`.
- [ADR-0023](ADR-0023-a-stage-is-a-deploy-time-environment-resolved-to-project-and-branch.md) — A stage is a deploy-time environment; the CLI resolves the app's Project (by root-system name) and the stage's Branch outside Alchemy before the stack runs.
