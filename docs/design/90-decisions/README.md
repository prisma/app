...
- [ADR-0029](ADR-0029-secrets-are-env-named-params.md) — A secret is an ordinary config param with the `secret` facet, bound to an explicit platform env-var name via `envSecret`; the framework carries only the name — pointer rows, preflight, and rotation follow from that. *(Proposed)*
