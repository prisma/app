...
- [ADR-0037](ADR-0037-service-rpc-calls-carry-an-idempotency-key.md) — Every service RPC call carries an `Idempotency-Key`: the client mints one per logical call and reuses it across a bounded retry; the server requires it, runs one call per key, and replays completed 2xx/4xx answers (never 5xx) from a bounded in-process store. Retrying is permanent protocol behavior, not a platform workaround, and there is no per-method opt-in — a flag would be an unverifiable claim. Handlers may read the key via an optional third argument for their own durable exactly-once.
