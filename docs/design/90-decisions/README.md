...
- [ADR-0033](ADR-0033-lowering-spi-seams-are-consumer-declared.md) — Every lowering-SPI seam's type is declared by its consumer, retiring the shared `LoweredNode` bag: descriptor-owned generic phase handoffs (`ServiceLowering<P, S>`), an extension-guarded `unknown` application seam, and name-keyed `WiringOutputs` resolved against the consumer's connection declaration; the lowering loop is the only router. Records the alchemy execution facts (phase handoffs legitimately carry unresolved `Output<T>`), that the heterogeneous registry's safety rests on the loop rather than the compiler, and that unchecked claims must be named, justified, and singular.
