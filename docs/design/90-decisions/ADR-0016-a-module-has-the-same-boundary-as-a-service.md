...
declares `@prisma/composer*` as peer dependencies, and its declared range asserts
...
`@prisma/composer/nextjs` because some installed module happens to use Next inside.
...
- Reusable modules are pack-specific: a module authored in `@prisma/composer-prisma-cloud`'s
