...
Prisma Composer splits into a **thin core** that knows only an abstract model, and
...
`@prisma/composer-prisma-cloud` for another target pack and the router is unchanged.
...
Prisma Composer does not bundle app code. Turning the service module into a runnable
...
The test of the split is a one-line swap: change `@prisma/composer-prisma-cloud` to another
target pack and nothing in `@prisma/composer` changes — not the abstract model, not the
...
- **Alchemy in core, or behind the target?** — *Resolved*: Alchemy is core's
  provisioning substrate (`@prisma/composer/deploy` imports it); the principle forbids
