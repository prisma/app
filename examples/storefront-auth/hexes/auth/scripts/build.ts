...
// self-contained runnables — each imports (and re-bundles) service.ts on its
// own; run() and load() hand off state through process.env, not shared JS
// object identity, so this duplication is by design (see serializer.ts).
// MakerKit ships no build step, but it does own the artifact envelope —
// bootstrap.js + compute.manifest.json + the deterministic tar are printed by
// the pack's `package()` at deploy, not here.
...
// The app's own runnable (the build adapter's `entry`): Hono + @makerkit/*
// inlined so it needs nothing but `bun` — a Compute runtime built-in,
