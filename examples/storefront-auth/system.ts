...
/**
 * The storefront-auth app: two services and their shared Postgres in one
 * system. The system owns the database and wires it into auth's `db` slot;
 * `auth` exposes an RPC contract; `storefront` consumes it (auth's `rpc`
 * port → storefront's `auth` slot, compat-checked). Transparent wiring,
 * executed at Load.
 *
