...
describe('findPnPinViolations', () => {
      ...
      findPnPinViolations(
        {
          ...
          peerDependencies: { '@prisma/composer-rpc': '0.7.0' },
        },
        INTERNAL_NAMES,
      ),
      [],
  ...
  it('flags a caret range in dependencies', () => {
    const v = findPnPinViolations(
      {
        name: '@scope/pkg',
        dependencies: { '@prisma/composer': '^0.7.0' },
      },
      INTERNAL_NAMES,
    );
  ...
  it('flags every common imprecise form', () => {
    for (const spec of ['^0.7.0', '~0.7.0', '>=0.7.0', '0.7.x', '*', '0.7.0 || 0.8.0']) {
      const v = findPnPinViolations(
        {
          name: '@scope/pkg',
          dependencies: { '@prisma/composer': spec },
        },
        INTERNAL_NAMES,
      );
  ...
  it('flags violations in peerDependencies and optionalDependencies, not just dependencies', () => {
    const v = findPnPinViolations(
      {
        name: '@scope/pkg',
        peerDependencies: { '@prisma/composer': '^0.7.0' },
        optionalDependencies: { '@prisma/composer-cloud': '~0.7.0' },
      },
      INTERNAL_NAMES,
    );
    ...
    const v = findPnPinViolations(
      {
        name: '@scope/pkg',
        dependencies: { '@prisma/composer': '0.7.0' },
        peerDependencies: { '@prisma/composer-cloud': '^0.7.0' },
      },
      INTERNAL_NAMES,
    );
    assert.equal(v.length, 1);
  ...
  it('does not double-report a workspace:/catalog: spec (delegated to the leak rule)', () => {
    const pkg = {
      name: '@scope/pkg',
      dependencies: {
        '@prisma/composer': 'workspace:0.7.0',
        '@prisma/composer-cloud': 'catalog:',
      },
    };
