...
//      every dep field.
//
//   2. Internal-workspace-package exact-pin check. Every dependency in
//      `dependencies`, `peerDependencies`, and `optionalDependencies` (the
//      three fields that ship to consumers; `devDependencies` is skipped)
//      whose name belongs to the workspace's own packages must be a single
//      exact version `X.Y.Z` (with an optional semver pre-release suffix).
...
//      this is the mechanism that gives every published internal package
//      an exact pin on its siblings.
//
//      Membership in "the workspace's own packages" is determined by
//      `pnpm list -r`, not by name prefix: the `@prisma/` npm scope is
...
// The internal-workspace-package exact-pin check looks only at the three
// fields that ship to consumers. devDependencies aren't installed by
// consumers so imprecise specs there don't affect the type-identity
// invariant the pin check enforces.
 ...
 * `(field, name, spec)` triples where a dependency on one of the
 * workspace's own packages (per `internalNames`) is not pinned to an
 * exact version. Specs already flagged by {@link isLeak} are skipped —
 * those are reported by the leak rule to avoid double-attribution
 * noise. Pure / side-effect-free.
 *
 * @param {Record<string, unknown>} pkgJson
 * @param {Set<string>} internalNames names of the workspace's own packages
 * @returns {Array<{ field: string; name: string; spec: string }>}
 */
export function findPnPinViolations(pkgJson, internalNames) {
  const violations = [];
...
function tarballNameFor(pkgName, version) {
  ...
  // (e.g. `@prisma/composer-cloud@1.2.3` → `prisma-composer-cloud-1.2.3.tgz`).
    ...
    if (json) {
      ...
      stderrWrite(
        '\n  [leak] specs are pnpm-internal (workspace:/catalog:) and break consumer installs.\n' +
          ...
          "  [pin]  every dependency on one of the workspace's own packages, in\n" +
